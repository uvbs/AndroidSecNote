...
#define TAG "TTT"
#define DEBUG_PRINT(...) __android_log_print(ANDROID_LOG_INFO,TAG ,__VA_ARGS__)
...
int main(int argc, char** argv) {
    ...
    if (argc != 3) {
        printf("usage: inject app_name so_name");
        return 1;
    }
    ...
    printf("injecting %s into %s", sn, pn);
    
    pid_t target_pid;
    target_pid = find_pid_of(pn);
    printf("target process's pid: %d\n",target_pid);
    int ret = inject_remote_process(target_pid, sn, "InjectInterface", (void*)"I'm parameter!", strlen("I'm parameter!") );
